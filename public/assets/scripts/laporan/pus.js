...
var PusIndex = function() {
    
	var select2Handler = function() {
		$('#periode_sensus').select2({
        ...
        $('#status_hamil').select2({
			placeholder: '--- Pilih Status Hamil ---',
            minimumResultsForSearch: -1,
			// allowClear: true
        });
    ...
    var showReport = function () {
        $('#btnShow').click(function (e) {
            e.preventDefault();
            if ($('#periode_sensus').val()=='') {
                swal.fire('', 'Periode Pendataan belum dipilih', 'error');
                return false;
            }
            if ($('#kelompok_umur').val()=='') {
                swal.fire('', 'Kelompok Umur belum dipilih', 'error');
                return false;
            }
            if ($('#status_hamil').val()=='') {
                swal.fire('', 'Status Hamil belum dipilih', 'error');
                return false;
            }
            
            var param = {
                periode_sensus: $('#periode_sensus').val(),
                kelompok_umur: $('#kelompok_umur').val(),
                status_hamil: $('#status_hamil').val(),
    ...
    var btnPrintHandler = function () {
        $('#btnPrint').click(function (e) {
            ...
            var url = base_url + '/laporan/pus/data?' + querystr;
